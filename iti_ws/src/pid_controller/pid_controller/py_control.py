...
class Twist_Pub_Node(Node):
    def __init__(self):
        ...
        # Target
        self.desired_x = float(input("Target x: "))  # Target x
        self.desired_y = float(input("Target y: "))  # Target y
        self.tolerance = float(
            input("Tolerance (try to keep a small number near ZERO): "))  # Tolerance
    ...
    def sub_call(self, msg):
        ...
        self.error_lin = sqrt(
            ((self.desired_x-self.now_x)**2)+((self.desired_y-self.now_y)**2))

        self.theta = atan2((self.desired_y-self.now_y),
                           (self.desired_x-self.now_x))
